...
JWT_EXPIRES_IN=1d
```
You can change `DB_*` and Docker will use those values.
...
│  ├─ models/
│  │  └─ user.ts                 # User model + initUserModel()
│  ├─ index.ts                   # boots server via ESM import
│  └─ server.ts                  # Express app + DB sync
...
- Initialize models in one place:
  - `src/database/index.ts` imports model init functions and calls them with the active `sequelize`
